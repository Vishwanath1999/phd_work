...
class MRR:
  def __init__(self, params):
    ...
    self.seta = self.seta[4000]*np.ones((len(self.seta)//10,1))
  ...
  def calc_beta2(self):
    ...
    self.theta = (np.linspace(-np.pi, np.pi, self.N) + D1*np.linspace(0, (len(self.seta)//10)*self.Tr, self.N))*theta_norm # normalized frequency detuning
    self.tau = 0.5*self.kappa*np.linspace(0, (len(self.seta)//10)*self.Tr, (len(self.seta)//10)) # normalized time coordinate tau = kappa*t/2
...
def lle_loss(model, T0, Theta0, u_init, v_init, seta_init, T, Theta, seta_array, F, cal=False, epsilon=100):
  ...
  # if fourier feature encoding is used then boundary conditions are already included in the collocation points
  # # define periodic boundary condition loss
  # e_lb = model(torch.cat((T_lb, Theta_lb, seta_b), dim=1))
  # u_lb = e_lb[:, 0:1]
  # v_lb = e_lb[:, 1:2]

  # e_ub = model(torch.cat((T_ub, Theta_ub, seta_b), dim=1))
  # u_ub = e_ub[:,0:1]
  # v_ub = e_ub[:,1:2]
...
training_params = {
    ...
    'mapping_size': 128, # Number of Fourier features,
...
def validate(network, T_val, Tau_val,seta_val, T_lb=None, Tau_lb=None, T_ub=None, Tau_ub=None, T0=None, Tau0=None):
    ...
    if ring1.seta.max() == ring1.seta.min():
        cax1 = ax1.imshow(np.abs(h), cmap='jet', origin='lower', aspect='auto', extent=[ring1.tau.min(), ring1.theta.max(), \
...
cal = False  # Causality-aware loss
...
for epsilon in training_params['epsilons']:
  ...
  for epoch in tqdm(range(epochs), ncols=120):
    ...
    if epoch % 1000 == 0:
      if cal:
        weights_fig = plot_temporal_weights(w_i, np.linspace(ring1.seta.min(), ring1.seta.max(), len(w_i)))
