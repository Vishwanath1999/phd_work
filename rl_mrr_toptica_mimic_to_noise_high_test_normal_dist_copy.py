...
def run_test_processes(run_id, save_dir):
    ...
    acav_hist = []
    while not done:
        action = 0#agent.choose_action(obs, True)
        env.current_del_omega = 2*torch.pi*detuning_smooth_tensor[idx//env.ctrl_freq]
        next_state, reward, done, terminal, _, acav_, ecav_, e_wg = env.step(state, 0, desired_spectrum_tensor)
...
if __name__ == '__main__':
    ...
    save_dir = os.path.join('./results', agent.run_name, env.thermal_effect,'test_smooth')
