...
class RL_MRR_Env():
    ...
    def spectral_symm_torch(spectrum: torch.Tensor, floor: float=-55) -> float:
        """
        Computes the spectral symmetry of the input spectrum (PyTorch tensor) by calculating
        the Pearson correlation coefficient between the left and right halves of the spectrum,
        after filtering out values below the specified floor threshold.
        Args:
            spectrum (torch.Tensor): Input spectrum as a 1D PyTorch tensor.
            floor (float): Threshold below which spectral values are ignored.
        Returns:
            float: Spectral symmetry value between -1 and 1.
        """
        n = spectrum.size(0)
        center_idx = n // 2
...
def run_test_processes(run_id, save_dir):
    ...
    obs = np.concatenate((ecav[:,len(env.mu)//2-150:len(env.mu)//2+150]/10,np.zeros((env.seq_len,1)), log_pcav[:,np.newaxis], bounds*np.ones((env.seq_len,1))),axis=1)
    ...
    while not done:
        ...
        bounds = calc_detuning_distance(env, scale=3)
        ecav_obs = np.concatenate((ecav_[-1,len(env.mu)//2-150:len(env.mu)//2+150]/10, 3*env.current_del_omega/(env.del_omega_ul - env.del_omega_end), 10*np.log10(curr_pcav)+30, bounds), axis=0)
