...
class DintFourierFeatureEmbedding(nn.Module):
  def __init__(self, dint, P_x=1):
    ...
    self.dint = torch.tensor(self.dint, device=self.device, dtype=torch.float32)
    self.register_buffer('B_x',self.dint)

  def forward(self, x):
    # Ensure input is on the same device as the random matrix
    x = x.to(self.device)

    x_proj = (2 * torch.pi * x) @ self.B_x
...
class PINN(nn.Module):
  def __init__(self, layers, mapping_size=64, scale=[10.0], use_fourier_features=True, use_rwf=False, P_x=1, P_t=1, mu=1.0, sigma=0.1, dint=None):
    ...
    if use_fourier_features:
      self.fourier_embedding = DintFourierFeatureEmbedding(dint, P_x=P_x)
      modified_layers = [2*len(dint)+1] + layers[1:]
    else:
  ...
  def forward(self, x):
    ...
    if self.use_fourier_features:
      x_f = self.fourier_embedding(x[:,1:2])
...
ring1_params =  {
                        ...
                        'Pin': 2, # Pump power [W]
                        'dseta_start': -10, #seta[seta_idx],
                        'dseta_stop': 45, #seta[seta_idx],
                        'dseta_step': 0.01
                    }
...
if wandb_:
    ...
    # set wandb run name
    wandb.run.name = 'LLE_PINN_MSFFN_dint_ffn'
# %%
# # generate tau and t
T0, Theta0, u_init, v_init, seta_init, T_lb, Theta_lb, T_ub, Theta_ub, T, Theta, seta_array, F = \
  ring1.gen_tau_t_lhs(N0=200,Nb=10, Nf=50000, seta_idx=3000)
print(T_lb.shape, Theta_lb.shape, T_ub.shape, Theta_ub.shape, T.shape, Theta.shape, seta_array.shape, F.shape, u_init.shape, v_init.shape)
...
network = PINN(layers=training_params['layers'],
               ...
               P_x=P_x,
               P_t=P_t,
               dint = ring1.dint[0:(len(ring1.dint)+1)//2]
               ).to(device)
