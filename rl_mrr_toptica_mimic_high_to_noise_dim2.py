...
# %%
config = {
    'input_dim': [env.seq_len, 300+5],
    'n_actions': 2,
    'alpha': 3e-4,
    'beta': 3e-4,
    'mem_size': int(1e6),
    'run_name': 'mrr_sac_cluster_delayed_toptica_pow_ton_un_norm_high_only_detuning_v7',
    'batch_size': 256,
...
def run_test_processes(run_id, save_dir):
    ...
    obs = np.concatenate((ecav[:,len(env.mu)//2-150:len(env.mu)//2+150]/10, log_pcav[:,np.newaxis], (env.power/den)*np.ones((env.seq_len,1)), np.zeros((env.seq_len,1)), bounds*np.ones((env.seq_len,1))),axis=1)
    ...
    while not done:
        ...
        bounds = calc_detuning_distance(env, scale=3)
        ecav_obs = np.concatenate((ecav_[-1,len(env.mu)//2-150:len(env.mu)//2+150]/10,10*np.log10(curr_pcav+1e-12)+30, env.power/den, 3*env.current_del_omega/(env.del_omega_ul - env.del_omega_end),  bounds), axis=0)
