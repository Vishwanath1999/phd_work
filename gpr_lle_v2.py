...
# Bounds for Ppmp only
# %%
# bounds = [(0.001, 0.5)]  # Ppmp in range [0.001, 0.5]

# # Initialize training data (only for Ppmp)
# x_train = torch.tensor([[0.01], [0.25]], dtype=torch.float32)  # Initial samples
# y_train = torch.tensor([compute_reward(x[0].item()) for x in x_train], dtype=torch.float32)
...
#     # Randomly initialize and optimize over the parameter space (Ppmp)
#     x_next = torch.tensor(np.random.uniform(*bounds[0], size=(1, 1)), dtype=torch.float32)
...
#     y_next = torch.tensor([compute_reward(x_next[0].item())], dtype=torch.float32)
...
# # Final results
# print(f"Optimal parameter: Ppmp = {x_train[y_train.argmin()].cpu().numpy()}, min loss: {y_train.min().item()}")
# %%
from multiprocessing import Pool, cpu_count

# Bounds for Ppmp
bounds = [(0.001, 0.3)]  # Ppmp in range [0.001, 0.5]
...
num_iterations = 50
num_parallel = cpu_count()  # Use all available CPU cores
print(f"Using {num_parallel} CPU cores for parallel optimization.")

def parallel_acquisition(params):
    """
    Wrapper for the acquisition function for parallel evaluation.
    """
    x, gp, best_y = params
    return acquisition_function(torch.tensor(x, dtype=torch.float32), gp, best_y).item()

def parallel_optimization_step(gp, x_train, y_train, num_samples=10):
    """
    Perform parallel optimization to find the next points.
    """
    best_y = y_train.max().item()  # Best reward observed so far
    candidate_points = np.random.uniform(bounds[0][0], bounds[0][1], (num_samples, 1))  # Random samples in the bounds

    # Prepare inputs for parallel processing
    params = [(candidate_points[i], gp, best_y) for i in range(len(candidate_points))]

    # Use multiprocessing to evaluate the acquisition function in parallel
    with Pool(num_parallel) as pool:
        acquisition_values = pool.map(parallel_acquisition, params)

    # Find the best point among the candidates
    best_idx = np.argmax(acquisition_values)
    x_next = torch.tensor(candidate_points[best_idx], dtype=torch.float32)
    return x_next

# Optimization loop
for i in tqdm(range(num_iterations), ncols=120, desc='Optimization Loop'):
    # Perform parallel optimization step to find the next best point
    x_next = parallel_optimization_step(gp, x_train, y_train)
