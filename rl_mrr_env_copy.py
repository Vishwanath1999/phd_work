...
class RL_MRR_Env():
    ...
    def reset(self):
        ...
        return self.state, Acav.numpy(), (10*torch.log10(torch.abs(Ecav)**2)+30).numpy()
    ...
    def step(self, state, action, desired_spectrum):
        ...
        reward = -torch.linalg.norm(Ecav-desired_spectrum, ord=2)
        ...
        return self.next_state, reward.numpy(), done, Acav.numpy(), (10*torch.log10(torch.abs(Ecav)**2)+30).numpy()
...
# # %%
# r_hist = []
# for _ in tqdm(range(50000),ncols=120):
#     state, reward, done, awg = env.step(state, 1, desired_spectrum)
...
# import matplotlib.pyplot as plt
# plt.plot(np.abs(ns), label='Next State')
# plt.legend()
# plt.show()
# plt.plot(np.abs(s), label='Initial State')
# plt.legend()
# plt.show()

# # %%
# # plot the ffts of s and ns
# s_fft = np.fft.fftshift(np.fft.fft(s))
# ns_fft = np.fft.fftshift(np.fft.fft(ns))

# # plot abs in dBm
# plt.plot(10*np.log10(np.abs(s_fft)**2)+30, label='Initial State')
# plt.plot(10*np.log10(np.abs(ns_fft)**2)+30, label='Next State')
# plt.legend()
# plt.show()
...
# plt.show()
# %%
from actor_critic import NewAgent, Agent
agent = Agent(alpha=1e-4, beta=1e-4, input_dims=env.DKS_init.shape, gamma=0.99, layer1_size=256, layer2_size=256, n_actions=3)
# agent = NewAgent(alpha=1e-3, input_dims=env.DKS_init.shape, gamma=0.99, layer1_size=256, layer2_size=256, n_actions=3)
# torch.autograd.set_detect_anomaly(True)
# load saved policy
# agent.actor_critic.load_state_dict(torch.load('policy_mrr_spec2.pth', weights_only=True))
# %%
score = 0
num_episodes = 20
score_history = []
for i in range(num_episodes):
    ...
    for _ in tqdm(range(150000),ncols=120):
        state_abs = ecav#torch.abs(state).numpy()
        try:
            action = agent.choose_action(state_abs)
        except ValueError as e:
            print('Error: ', e)
            print('state_abs: ', state_abs)
            print('action: ', action)
        # print('chosen action: ', action)
        state_, reward, done, acav_, ecav_ = env.step(state,action, desired_spectrum)
        agent.learn(ecav, reward, ecav_, done)
        state = state_
        ecav = ecav_
        score += reward
...
plt.savefig('./rl_results/ac_score_history_spec2.png')
plt.show()

# %% run one eval episode freeze the weights of policy
...
score = 0
acav_hist = []
acav_hist.append(acav)
for idx in tqdm(range(200000),ncols=120):
    state_abs = ecav#torch.abs(state).numpy()
    action = agent.choose_action(state_abs)
    state_, reward, done, acav_, ecav_ = env.step(state,action, desired_spectrum)
    state = state_
...
plt.savefig('./rl_results/ac_ecav_hist_spec2.png')
plt.show()

# # imshow ifft of the ecav also 
plt.figure(figsize=(14,4))
...
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.savefig('./rl_results/ac_ecav_hist_ifft_spec2.png')
plt.show()
# %%
...
plt.grid()
plt.savefig('./rl_results/ac_rewards_spec2.png')
plt.show()

# plot and save ecav using ylines plot. 
desired_spectrum_dBm = 10*torch.log10(torch.abs(desired_spectrum)**2)+30
plt.figure(figsize=(14,4))
...
plt.savefig('./rl_results/ac_ecav_spec2.png')
plt.show()

# %%
# plot and save the actions with different colors
plt.figure(figsize=(10, 6))
...
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.savefig('./rl_results/ac_actions_spec2.png')
plt.show()

# %%
# save the model
torch.save(agent.actor_critic.state_dict(), './rl_results/ac_policy_mrr_spec2.pth')
# bring the policy network to cpu and save it as jitted model
# agent.policy.to('cpu')
# agent.policy = torch.jit.script(agent.policy)
# torch.jit.save(agent.policy, 'ac_mrr_spec_jit.pth')