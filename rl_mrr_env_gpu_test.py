...
class RL_MRR_Env():
    ...
    def __init__(self, seq_len=50, p_max=0.3, p_min=0.1, ctrl_freq=100, thermal_effect='moderate', 
        ...
        self.sim['DKS_init'] = self.sim['DKS_init'].astype(np.complex64)
        ...
        if self.sim_tensor['DKS_init'].dtype == torch.complex64 or self.sim_tensor['DKS_init'].dtype == torch.complex64:
        ...
        h_R_fft = (h_R * dt_fast).to(torch.complex64)
    ...
    def tensor_to_dict(self,tensor):
        ...
        for key in tensor:
            ...
            if isinstance(tensor[key], torch.Tensor):
                if tensor[key].dtype == torch.complex64:
    ...
    def _gaussian_kernel1d(self, fwhm_bins: float, radius: int = None) -> torch.Tensor:
        ...
        sigma = max(float(fwhm_bins) / 2.355, 1e-6)
        if radius is None:
    ...
    def _smooth1d(self, y: torch.Tensor, k: torch.Tensor) -> torch.Tensor:
        # y: (N,)
        pad = (k.numel() - 1) // 2
        y2 = y[None, None, :]  # (1,1,N)
        k2 = k[None, None, :]  # (1,1,K)
        ypad = torch.nn.functional.pad(y2, (pad, pad), mode="reflect")
        return torch.nn.functional.conv1d(ypad, k2).squeeze(0).squeeze(0)
    
    def measure_osa_dbm(self, E_spec: torch.Tensor) -> torch.Tensor:
        # E_spec is complex spectrum (e.g. Ewg)
        P_w = torch.abs(E_spec) ** 2  # assumes |E|^2 is in W (matches your +30 dBm conversion)
    
        # RBW effect (instrument resolution / leakage across bins)
        if self.osa_rbw_fwhm_bins is not None and self.osa_rbw_fwhm_bins > 0:
            P_w = self._smooth1d(P_w, self._osa_rbw_kernel)
    
        # Add OSA noise floor in linear power (W)
        P_w = P_w + self.osa_noise_floor_w
    
        # Convert to dBm
        P_w = torch.clamp(P_w, min=1e-18)
        y_dbm = 10.0 * torch.log10(P_w) + 30.0
    
        # Optional extra read noise (often looks like ~constant dB jitter)
        if self.osa_read_noise_db_std > 0:
            y_dbm = y_dbm + self.osa_read_noise_db_std * torch.randn_like(y_dbm)
    
        # ADC effects: clip + quantize
        if self.adc_enable:
            y_dbm = torch.clamp(y_dbm, min=self.adc_min_dbm, max=self.adc_max_dbm)
            if self.adc_bits is not None:
                levels = (2 ** int(self.adc_bits)) - 1
                step = (self.adc_max_dbm - self.adc_min_dbm) / max(levels, 1)
            else:
                step = float(self.adc_lsb_db)
            y_dbm = torch.round((y_dbm - self.adc_min_dbm) / step) * step + self.adc_min_dbm
    ...
    def step(self, state, action, desired_spectrum):
        ...
        for ii in range(1):
            self.Ein[ii,int(self.mu0+self.ind_pmp[ii])] = torch.sqrt(Ppmp[ii])*len(self.mu)
            self.Ain[ii] = torch.fft.ifft(torch.fft.fftshift(self.Ein[ii],dim=0),dim=0)*torch.exp(-1j*(self.phi_pmp[ii]+ int(self._use_phase_noise)*self.phase_noise[ii]))
...
def run_test_processes(run_id, save_dir):
    ...
    obs = np.concatenate((ecav[:,len(env.mu)//2-150:len(env.mu)//2+150]/10, log_pcav[:,np.newaxis],  np.zeros((env.seq_len,1)), bounds*np.ones((env.seq_len,1))),axis=1)
    ...
    while not done:
        ...
        bounds = calc_detuning_distance(env, scale=3)
        ecav_obs = np.concatenate((ecav_[-1,len(env.mu)//2-150:len(env.mu)//2+150]/10, 10*np.log10(curr_pcav+1e-12)+30, 3*env.current_del_omega/(env.del_omega_ul - env.del_omega_end),  bounds), axis=0)
