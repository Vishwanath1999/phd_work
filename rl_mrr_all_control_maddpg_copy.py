...
class RL_MRR_Env():
    ...
    def step(self, state, action, desired_spectrum):
        ...
        Ppmp1 = Ppmp
        Ppmp = torch.clamp(Ppmp, min=0, max=1)
        ...
        reward = pcav_mse+1#self.compute_reward(spectral_mse, pcav_mse, alpha=self.alpha_)
        # if Ppmp > 2 and Ppmp <0:
        #     reward -= 5
        
        # reward = np.array([spectral_mse.item(), pcav_mse[0], self.alpha_])
        ...
        # if Ppmp1[0] > 0.5 or Ppmp1[0] <0.01:
        #     done = True
...
# from matd3pg import MATD3PG as Agent
# agent = Agent(alpha=1e-4, beta=1e-4, input_dims=[441], disc_n_actions=3, cont_n_actions=1,\
#               warmup=10000, batch_size=256, tau=0.001)

from td3pg_param import Agent
agent = Agent(alpha=1e-4, beta=1e-4, input_dims=[441], tau=0.001, gamma=0.99, \
              n_actions_cont=1, n_actions_disc=3, max_size=1000000, layer1_size=256, layer2_size=256,\
                  batch_size=128, use_grad_inv=True, warmup=10000)
# %%
# init wandb run
...
wandb.watch(agent.critic_1, log='gradients', log_freq=1000)
# set the wandb run name
wandb.run.name = 'param_td3pg'
# %% MADDPG train loop
logs={}
n_games = 50000
# r_hist = []
...
for i in range(n_games):
    ...
    agent.noise.reset()
    while not done:
        action, cont_action, disc_action = agent.choose_action(ecav/10, global_n_steps)
        # print(action)
        # a, b = 0.001, 0.5  # Define action bounds
        # scaled_action = a + 0.5 * (action[0] + 1) * (b - a)
        # scaled_action = np.clip(scaled_action, a, b)
        perf_action = action#np.array([np.argmax(action[1]), scaled_action[0]])
        ...
        if achieved==True:
            ...
            new_done = done    
        logs['reward'] = reward      
        agent.remember(ecav/10, cont_action, disc_action, reward, ecav_/10, new_done)
        state = next_state
        ecav = ecav_
        score += reward
        ...
        if agent.memory.mem_cntr > 4*agent.batch_size:
            # print('Training...')
            al, cl = agent.learn(global_n_steps)
...
# for idx in tqdm(range(env.init_steps_, env.max_steps), ncols=120):
    ...
    try:
        det = np.random.choice([0, 1, 2], p=[1/3, 1/3, 1/3])
        pow_ = np.random.uniform(0.01, 0.5)
