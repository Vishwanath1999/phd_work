...
class FourierFeatureEmbedding(nn.Module):
  def __init__(self, mapping_size, scales=[5.0], P_x=1):
  ...
  def forward(self, x):
    ...
    x_proj = (2 * torch.pi * x[:,0:1] / self.P_x) @ self.B_x
...
class PINN(nn.Module):
  def __init__(self, layers, mapping_size=64, scale=[10.0], use_fourier_features=True, use_rwf=False, \
    ...
    if use_fourier_features:
      self.fourier_embedding = FourierFeatureEmbedding(mapping_size, scale, P_x)
  ...
  def forward(self, x):
    ...
    t_x_f = torch.cat([x[:,0:1], x_f, x[:,2:3]], dim=1)
