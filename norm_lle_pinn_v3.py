...
class MRR:
  ...
  def gen_tau_t_lhs(self, N0=100, Nb=100, Nf=20000):
    ...
    T_sampled = np.min(self.tau) + (np.max(self.tau) - np.min(self.tau)) * lhs_samples[:, 0][:, np.newaxis]
    Theta_sampled = np.min(self.theta) + (np.max(self.theta) - np.min(self.theta)) * lhs_samples[:, 1][:, np.newaxis]
  ...
  def gen_tau_t_lhs_time_march(self, N0=100, Nb=100, Nf=20000, M=5):
    ...
    for i in range(M):
        ...
        # Rescale LHS samples to match the range of the meshgrid for T and Theta
        T_sampled = t_min + (t_max - t_min) * lhs_samples[:, 0][:, np.newaxis]
        Theta_sampled = np.min(self.theta) + (np.max(self.theta) - np.min(self.theta)) * lhs_samples[:, 1][:, np.newaxis]
...
def calc_w_i(residual_losses,unique,inverse_indices,epsilon):
    ...
    residual_losses = residual_losses.view(-1)
...
def lle_loss(model,prev_model, T0, Theta0, u_init, v_init, T_lb, Theta_lb, T_ub, Theta_ub, T, Theta, seta_array, F, M, curr_step=0, cal=False, epsilon=100):
  ...
  if curr_step==0:
    ...
    # pick T0 and Theta0 from the last element of the previous step of T and Theta splits
    ids = torch.argsort(T,dim=0)
    T_ = T[ids]
    T0 = torch.split(T_.squeeze(), len(T) // M)[curr_step-1][-1]*torch.ones_like(T0)
    with torch.no_grad():
      e_init = prev_model(torch.cat((T0, Theta0), dim=1))
    u_init_pred = e_init[:,0:1]
    v_init_pred = e_init[:,1:2]
  ...
  if cal:
    ...
    loss_pde = loss_pde[ids].flatten()
    sorted_T = T[ids]
    unique, inverse_indices = torch.unique(sorted_T, return_inverse=True)
    w_i, break_ = calc_w_i(loss_pde, unique, inverse_indices, epsilon)
    loss_pde_splits = torch.split(loss_pde, len(loss_pde) // M)
    w_i_splits = torch.split(w_i, len(w_i) // M)
    # calculate weighted loss for curr_step
    loss_pde = torch.mean(loss_pde_splits[curr_step] * w_i_splits[curr_step])
    norm_pde = torch.norm(loss_pde_splits[curr_step], p=2)
    if w_i_splits[curr_step].min() > 0.99:
      break_ = True
    else:
      break_ = False
  else:
  ...
  return loss_b, loss_pde, loss_init, lambda_b, lambda_pde, lambda_init, w_i.detach().cpu().numpy(), break_
...
# %%
# # generate tau and t
T0, Theta0, u_init, v_init, T_lb, Theta_lb, T_ub, Theta_ub, T, Theta, seta_array, F = ring1.gen_tau_t_lhs(N0=200,Nb=500, Nf=40000)
...
train_data = {'T0': T0, 'Theta0': Theta0, 'u_init': u_init, 'v_init': v_init, 'T_lb': T_lb, 'Theta_lb': Theta_lb, \
              'T_ub': T_ub, 'Theta_ub': Theta_ub, 'T_sampled': T, 'Theta_sampled': Theta, 'seta_array': seta_array, 'F': F}
...
M = training_params['M']
cal = True
lambda_pde, lambda_b, lambda_init = 1, 1, 100
# Convert data to tensors and move them to the specified device (CPU/GPU)
data_batch = to_tensor_device(train_data, device)
for t_step in range(M):
  print(f'Running for time step: {t_step}')
  for eps in training_params['epsilons']:
    ...
    for epoch in tqdm(range(training_params['epochs']),ncols=120):
      ...
      T0, Theta0 = data_batch['T0'], data_batch['Theta0']
      if t_step == 0:
        u_init, v_init = data_batch['u_init'], data_batch['v_init']
      T_lb, Theta_lb = data_batch['T_lb'], data_batch['Theta_lb']
      T_ub, Theta_ub = data_batch['T_ub'], data_batch['Theta_ub']
      T, Theta = data_batch['T_sampled'], data_batch['Theta_sampled']
      seta_array, F = data_batch['seta_array'], data_batch['F']

      # Zero the gradients
      optimizer.zero_grad()
      loss_b, loss_pde, loss_init, _, _, _, w_i, break_ = lle_loss(network,prev_model, T0, Theta0, u_init, v_init, T_lb, Theta_lb, T_ub, \
                                                                                             Theta_ub, T, Theta, seta_array, F, M, t_step, cal, eps)
      loss = lambda_pde*loss_pde + lambda_init*loss_init + lambda_b*loss_b
      loss.backward()
      # clip gradients
      ...
      if epoch % 1000 == 0:
        # print(f'Epoch: {epoch}, Loss: {loss.item()}, Loss_b: {loss_b.item()}, Loss_pde: {loss_pde.item()}, Loss_init: {loss_init.item()}')
        # # print current learning rate
        # print(f'Current LR: {scheduler.get_last_lr()[0]}')
        if cal:
          fig = plot_temporal_weights(w_i, x =np.linspace(ring1.seta.min(), ring1.seta.max(), len(w_i)))
          logs['Temporal weights for'+ r"$\epsilon$:"+str(eps)] = wandb.Image(fig)
          plt.close(fig)
      ...
      if break_:
        print(f'Breaking at time step: {t_step}')
        break
      
      wandb.log(logs)
      # check if logs has the keys
