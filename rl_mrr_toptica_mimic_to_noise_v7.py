...
if config['train']:
    ...
    for game in range(n_games):
        ...
        while not done:
            ...
            bounds = calc_detuning_distance(env, scale=3)
            ecav_obs = np.concatenate((ecav_[-1,len(env.mu)//2-150:len(env.mu)//2+150]/10, 10*np.log10(curr_pcav)+30, 3*env.current_del_omega/(env.del_omega_ul - env.del_omega_end), bounds), axis=0)
