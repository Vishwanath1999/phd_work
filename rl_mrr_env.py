...
agent = PolicyGradientAgent(ALPHA=0.001, input_dims=[441], GAMMA=0.99, n_actions=3, layer1_size=256, layer2_size=256)
train = True
# load saved policy
if train == False:
    agent.policy.load_state_dict(torch.load('./rl_results/policy_mrr_spec2.pth', weights_only=True))
# %%
if train:
    score = 0
    num_episodes = 40
    ...
    plt.savefig('./rl_results/score_history_spec3.png')
    plt.show()
...
for _ in tqdm(range(200000),ncols=120):
    state_abs = ecav#torch.abs(state).numpy()
    action = agent.choose_action(state_abs)
    state_, reward, done, acav_, ecav_ = env.step(state,action, desired_spectrum)
    state = state_
...
plt.savefig('./rl_results/ecav_hist_spec3.png')
plt.show()
...
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.savefig('./rl_results/ecav_hist_ifft_spec3.png')
plt.show()
# %%
...
plt.grid()
plt.savefig('./rl_results/rewards_spec3.png')
plt.show()
...
plt.savefig('./rl_results/ecav_spec3.png')
plt.show()
...
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.savefig('./rl_results/actions_spec3.png')
plt.show()

# %%
# save the model
if train:
    torch.save(agent.policy.state_dict(), './rl_results/policy_mrr_spec3.pth')
# bring the policy network to cpu and save it as jitted model
# agent.policy.to('cpu')
# agent.policy = torch.jit.script(agent.policy)
# torch.jit.save(agent.policy, 'policy_mrr_spec_jit.pth')