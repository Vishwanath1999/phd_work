...
class RL_MRR_Env():
    ...
    def __init__(self, seq_len=50, p_max=0.3, p_min=0.1, ctrl_freq=100, thermal_effect='moderate', 
        ...
        self.del_omega_ul = 3*self.un_norm_kappa
        self.current_del_omega = torch.zeros(len(fpmp), device=DEVICE)
        self.current_del_omega[0] = del_omega_init
        ...
        for ii in range(len(fpmp)):
            ...
            Ain[ii] = torch.fft.ifft(torch.fft.fftshift(Ein[ii],dim=0),dim=0)*torch.exp(-1j*phi_pmp[ii]) # counter-propagating pump aux pump
        ...
        self.del_omega_init_aux = 2*self.un_norm_kappa  # Start at 3x kappa
        self.del_omega_end_aux = 0*self.un_norm_kappa  # End at 0
        self.current_aux_del_omega = 0*self.un_norm_kappa
        self.delta_omega_max_aux = self.delta_omega_max/2
        self.delta_omega_step_aux = self.delta_omega_step
        self.delta_omega_min_aux = self.delta_omega_min/2
    ...
    def FFT_Lin(it: int, alpha: torch.Tensor, Dint_shift: torch.Tensor, del_omega_all: torch.Tensor, tR: torch.Tensor) -> torch.Tensor:
        ...
        '''
        return (-alpha / 2) + 1j * (Dint_shift - del_omega_all[0]) * tR
    ...
    def reset(self, steps=None):
        ...
        self.current_del_omega[0] = self.del_omega_0
        self.delta_theta = torch.tensor(0.0, device=DEVICE, dtype=torch.float64)
        self.step_cntr = 0
        self.pcav_hist = []
        self.t_sim_step = 0
        ...
        for idx in range(self.init_steps_):
            ...
            del_omega = self.current_del_omega[0] + mul_factor*(1/self.Nt)*(self.del_omega_end - self.del_omega_init)
            self.current_del_omega[0] = del_omega
    ...
    @staticmethod
    def spectral_width_dbm_torch(y_dbm: torch.Tensor, ind_pmp: torch.Tensor, threshold: float = -60, center_idx: int = None, norm: int = 300) -> torch.Tensor:
        """
        ...
        mask[center_idx + ind_pmp[1]] = False  # exclude aux pump mode
    ...
    @staticmethod
    def any_above_threshold_excluding_center(tensor, threshold, ind_pmp):
        n = tensor.size(0)
        center_idx = n // 2
        # Create a mask to exclude the central mode and modes in ind_pmp
        mask = torch.ones(n, dtype=bool)
        mask[center_idx] = False  # Exclude central mode
        for mode in ind_pmp:
            mode_idx = center_idx + mode  # Calculate the index of the mode
            if 0 <= mode_idx < n:  # Ensure the mode index is within bounds
                mask[mode_idx] = False  # Exclude the mode
        tensor_exc_modes = tensor[mask]
        return torch.any(tensor_exc_modes > threshold).item()
    ...
    def step(self, state, action, desired_spectrum):
        ...
        '''
        env.power[0:1] = self.rescale_power(action[0:1], lower_limit=self.p_min, upper_limit=self.p_max, step_size=0.001)
        ...
        delta_omega_aux = (2*torch.pi)*self.rescale_and_quantize(action[2], self.delta_omega_min_aux, self.delta_omega_max_aux, self.delta_omega_step_aux)  # Convert GHz to rad/s
        
        for _ in range(self.ctrl_freq):
            del_omega = self.current_del_omega[0] + delta_omega 
            delta_omega_aux = self.current_del_omega[1] + delta_omega_aux

            if self.soft_clamp_ == False:
                self.current_del_omega[0] = torch.clamp(del_omega, min=min(self.del_omega_end, self.del_omega_ul), max=max(self.del_omega_end, self.del_omega_ul))
                self.current_del_omega[1] = torch.clamp(delta_omega_aux, min=min(self.del_omega_end_aux, self.del_omega_init_aux), max=max(self.del_omega_end_aux, self.del_omega_init_aux))
            else:
        ...
        if self.any_above_threshold_excluding_center(Ecav_dBm, threshold=-56, ind_pmp =self.ind_pmp) == False:
            reward = 0.5*np.mean(self.pcav_hist)
        else:
            reward = 2*env.spectral_width_dbm_torch(Ecav_dBm, self.ind_pmp) + 2*env.spectral_corr_torch(Ecav_dBm, desired_spectrum_dBm) + 2*(env.spectral_symm_torch(Ecav_dBm)) #+ env.calculate_proximity_bonus_v2(self.r_hist)
...
def calc_detuning_distance(env, scale=1):
    ...
    d_low = (env.current_del_omega[0] - env.del_omega_end)/span
    d_high = (env.del_omega_ul - env.current_del_omega[0])/span
...
config = {
    ...
    'beta': 3e-4,
    'mem_size': int(1e6),
    'run_name': 'mrr_sac_cluster_delayed_toptica_pow_ton_un_norm_high_multi_pmpv2',
    'batch_size': 128,
...
if config['train']:
    ...
    for i in range(n_games):
        ...
        obs = np.concatenate((ecav[:,len(env.mu)//2-150:len(env.mu)//2+150]/10,env.power[0:1]*np.ones((env.seq_len,1))/den,np.zeros((env.seq_len,1)), log_pcav[:,np.newaxis], bounds*np.ones((env.seq_len,1)), 100*env.delta_theta.item()*np.ones((env.seq_len,1))),axis=1)
        ...
        while not done:
            ...
            logs['power (W)'] = env.power[0]
            logs['detuning (MHz)'] = env.rescale_and_quantize(action[1], env.delta_omega_min, env.delta_omega_max, env.delta_omega_step)*1e-6
            logs['detuning aux (MHz)'] = env.rescale_and_quantize(action[2], env.delta_omega_min_aux, env.delta_omega_max_aux, env.delta_omega_step_aux)*1e-6
            logs['reward'] = reward  
            curr_pcav = np.sum(np.abs(acav_)**2,keepdims=True)
            
            bounds = calc_detuning_distance(env, scale=3)
            ecav_obs = np.concatenate((ecav_[-1,len(env.mu)//2-150:len(env.mu)//2+150]/10, env.power[0:1]/den, 3*env.current_del_omega[0]/(env.del_omega_ul - env.del_omega_end), 10*np.log10(curr_pcav)+30, bounds, 100*env.delta_theta.item()*np.ones((1,))), axis=0)
            ...
            det_hist.append(env.current_del_omega[0].item()/(2*np.pi*1e9))  # Convert rad/s to GHz
