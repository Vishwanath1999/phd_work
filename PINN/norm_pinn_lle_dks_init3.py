...
class MRR:
  ...
  def calc_beta2(self):
    ...
    self.theta = (np.linspace(0, 2*np.pi, self.N)-D1*np.linspace(0, len(self.seta)*self.Tr, self.N))
    self.theta = (self.theta+np.abs(self.theta).max()/2)*theta_norm # normalized frequency detuning (- D1*np.linspace(0, len(self.seta)*self.Tr, self.N))
...
class PINN(nn.Module):
  def __init__(self, layers, mapping_size=64, scale=[10.0], use_fourier_features=True, use_rwf=False, P_x=1, P_t=1, mu=1.0, sigma=0.1):
    ...
    if use_fourier_features:
      self.fourier_embedding = FourierFeatureEmbeddingv2(mapping_size, P_x, P_t)
      modified_layers = [2 * mapping_size + 2] + layers[1:]#[2 * mapping_size*(len(scale)+1)] + layers[1:]
    else:
...
seta = np.round(seta, 2)
seta_idx = 1#np.where(seta==0)[0][0]
ring1_params =  {
...
training_params = {
    ...
    'mapping_size': 10, # Number of Fourier features,
    ...
    'epsilons':[100], # epsilon values for causality aware loss
    'epochs':500000
}
wandb_ = True
...
if wandb_:
    ...
    # set wandb run name
    wandb.run.name = 'LLE_PINN_MSFFN_seq_ffn'
# %%
# # generate tau and t
...
# %% Dint ff visualize
dintff = FourierFeatureEmbeddingv2(15, P_x,P_t)
...
plt.subplot(1, 2, 2)
plt.scatter(T,Theta,c=transformed_input2[:,10],cmap='jet',label='Transformed')
plt.xlabel('T')
plt.ylabel('Theta')
