...
class RL_MRR_Env():
    ...
    def is_terminal(self, Ecav_dBm, desired_spectrum_dBm, achieved):
        ...
        if self.step_cntr-self.init_steps_ >= int(0.7*self.Nt) and corr < 0.25: #and self.step_cntr-self.init_steps_ <= self.Nt:
            terminal = True
            reward_penalty = -5
...
def run_test_processes(run_id, save_dir):
    ...
    while not done:
        ...
        delta_theta.append(env.delta_theta.item()/(2*np.pi*env.tR.item()))
    ...
    if terminal == False:
        ...
        # save all the data into npy files
        np.save(os.path.join(save_dir, str(run_id) + '_acav_hist.npy'), np.array(acav_hist))
        np.save(os.path.join(save_dir, str(run_id) + '_e_wg_hist.npy'), np.array(e_wg_hist))
        np.save(os.path.join(save_dir, str(run_id) + '_r_hist.npy'), np.array(r_hist))
        np.save(os.path.join(save_dir, str(run_id) + '_det_hist.npy'), np.array(det_hist))
        np.save(os.path.join(save_dir, str(run_id) + '_delta_theta.npy'), np.array(delta_theta))
        np.save(os.path.join(save_dir, str(run_id) + '_action_hist.npy'), np.array(action_hist))
