...
class RL_MRR_Env():
    ...
    def __init__(self, seq_len=50, p_max=0.3, p_min=0.1, ctrl_freq=100, thermal_effect='moderate'):
        ...
        self.ou_theta = 0.001  # adjust as needed
        self.ou_mu = 0.0
        self.ou_sigma = torch.sqrt(2*torch.pi* (self.pump_linewidth / self.FSR))
    ...
    def step(self, state, action, desired_spectrum):
        ...
        self.phase_noise += self.ou_theta * (self.ou_mu - self.phase_noise) * 1 \
            + self.ou_sigma * torch.randn(len(Ppmp), device=DEVICE, dtype=torch.float64) * np.sqrt(1)
...
if idx > int(0.5*env.max_steps):
    plt.savefig(os.path.join(save_dir, mod_pow + '_'+ '_pcav_spec_all_ctrl.png'))
    # save as svg also
    plt.savefig(os.path.join(save_dir, mod_pow + '_'+ '_pcav_spec_all_ctrl.svg'), format='svg')
plt.show()
# %%
...
plt.tight_layout()
if idx > int(0.5*env.max_steps):
    plt.savefig(os.path.join(save_dir, mod_pow + '_'+ '_ecav_hist_spec_all_ctrl.png'))
    plt.savefig(os.path.join(save_dir, mod_pow + '_'+ '_ecav_hist_spec_all_ctrl.svg'), format='svg')
plt.show()
...
if idx > int(0.5*env.max_steps):
    ...
    # save as svg also
    plt.savefig(os.path.join(save_dir, mod_pow +'_ecav_hist_ifft_spec_all_ctrl.svg'), format='svg')
plt.show()
# %%
plt.figure(figsize=(14,4))
...
plt.tight_layout()
if idx > int(0.5*env.max_steps):
    plt.savefig(os.path.join(save_dir, mod_pow + '_ewg_hist_spec_all_ctrl.png'))
    # save as svg also
    plt.savefig(os.path.join(save_dir, mod_pow + '_ewg_hist_spec_all_ctrl.svg'), format='svg')
plt.show()
# %% Reward Plot
plt.figure(figsize=(10, 6))
plt.plot(r_hist)
...
if idx > int(0.5*env.max_steps):
    ...
    # save as svg also
    plt.savefig(os.path.join(save_dir, mod_pow + '_rewards_spec_all_ctrl.svg'), format='svg')
plt.show()
# %%
...
if idx > int(0.5*env.max_steps):
    ...
    # save as svg also
    plt.savefig(os.path.join(save_dir, mod_pow + '_ecav_spec_all_ctrl_modes.svg'), format='svg')
plt.show()
...
if idx > int(0.5*env.max_steps):
    ...
    # save as svg also
    plt.savefig(os.path.join(save_dir, mod_pow + '_ecav_spec_all_ctrl_freq.svg'), format='svg')
plt.show()
# %%
...
if idx > int(0.5*env.max_steps):
    ...
    # save as svg also
    plt.savefig(os.path.join(save_dir, mod_pow + '_actions_spec_all_ctrl.svg'), format='svg')
plt.show()
...
if idx > int(0.5*env.max_steps):
    ...
    # save as svg also
    plt.savefig(os.path.join(save_dir, mod_pow + '_actions_power_spec_all_ctrl.svg'), format='svg')
plt.show()
# %%
plt.figure(figsize=(10, 6))
...
plt.tight_layout()
if idx > int(0.5*env.max_steps):
    plt.savefig(os.path.join(save_dir, mod_pow + '_delta_theta_spec_all_ctrl.png'))
    # save as svg also
    plt.savefig(os.path.join(save_dir, mod_pow + '_delta_theta_spec_all_ctrl.svg'), format='svg')
plt.show()
'''
# %%
