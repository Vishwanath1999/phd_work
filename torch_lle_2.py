...
# %%
# Function for the Linear FFT operator
@torch.jit.script
...
# @torch.jit.script
# def Fdrive_jit(it:int, theta:torch.Tensor, Ain:torch.Tensor, del_omega_all:torch.Tensor, Dint:torch.Tensor, 
#                mu0:int, ind_pmp:torch.Tensor, t_sim:torch.Tensor, fpmp:torch.Tensor, device: torch.device
#                ) -> torch.Tensor:
#     Force = 0*torch.exp(1j*theta)
#     for ii in range(len(fpmp)):
#         if ii > 0:
...
def ssfm_step(A0: torch.Tensor, it: int, alpha: torch.Tensor, Dint_shift: torch.Tensor,
              ...
              max_iter: int, tol: float, dt: int, kext: torch.Tensor, Fdrive_val:torch.Tensor) -> torch.Tensor:
    ...
    for _ in range(max_iter):
        ...
        err = torch.norm(A_prop - A_h_prop, 2) / torch.norm(A_h_prop, 2)
        if err < tol:
            return A_prop
        A_h_prop = A_prop
    err = torch.norm(A_prop - A_h_prop, 2) / torch.norm(A_h_prop, 2)
...
def MainSolver(Nt, saved_data, u0):
    ...
    for it in tqdm(range(Nt), ncols=120):
        ...
        u0 = ssfm_step(u0, it, alpha, Dint_shift, del_omega_all, tR, gamma, L, 
                       max_iter, tol, dt, kext, Fdrive_val)
        if it*num_probe/Nt > param['probe']:
...
# %%
plt.figure(figsize=(10,3))
plt.imshow(np.abs(Acav).T, aspect='auto', cmap='jet')#, extent=[detuning_gHz[0], detuning_gHz[-1], -tr/2, tr/2])
