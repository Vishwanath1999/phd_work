...
class MRR:
  ...
  def calc_beta2(self):
    ...
    t = np.linspace(0, len(self.seta)*self.Tr, len(self.seta))
    self.theta = (np.linspace(-np.pi, np.pi, self.N) - D1*np.linspace(0, len(self.seta)*self.Tr, self.N)) # normalized frequency detuning (- D1*np.linspace(0, len(self.seta)*self.Tr, self.N))
    self.theta = (self.theta)*theta_norm
    self.tau = 0.5*self.kappa*t # normalized time coordinate tau = kappa*t/2
  ...
  def gen_tau_t(self, N0=100, Nb=100, Nf=20000, seta_idx=1):
    ...
    # u_init = np.sqrt(2*self.g/self.kappa)*np.random.rand(N0)
    u_init = np.real(data_ifft[idx,seta_idx:seta_idx+1])
    # v_init = np.sqrt(2*self.g/self.kappa)*np.random.rand(N0)
  ...
  def gen_tau_t_lhs(self, N0=100, Nb=100, Nf=20000, seta_idx=1):
    ...
    u_init = np.sqrt(2*self.g/self.kappa)*np.random.rand(len(Theta0),1)
    v_init = np.sqrt(2*self.g/self.kappa)*np.random.rand(len(Theta0),1)
...
class FourierFeatureEmbedding(nn.Module):
  def __init__(self, mapping_size, scales=[5.0], P_x=1, P_t=1, P_zeta=1):
    ...
    self.register_buffer('B_zeta', torch.cat([scale * torch.randn((1, mapping_size), device=self.device) for scale in scale_1], dim=1))
  ...
  def forward(self, x):
    ...
    zeta_proj = (2 * torch.pi * x[:, 2:3] / self.P_zeta) @ self.B_zeta
    ...
    return torch.cat([torch.cos(t_proj), torch.sin(t_proj)],dim=1), torch.cat([torch.cos(x_proj), torch.sin(x_proj)], dim=1), torch.cat([torch.cos(zeta_proj), torch.sin(zeta_proj)], dim=1)

class PINN(nn.Module):
  def __init__(self, layers, mapping_size=64, scale=[10.0], use_fourier_features=True, use_rwf=False,\
                P_x=1, P_t=1, P_zeta=1, mu=1.0, sigma=0.1):
    ...
    if use_fourier_features:
      self.fourier_embedding = FourierFeatureEmbedding(mapping_size, scale, P_x, P_t, P_zeta)
      modified_layers = [2 * mapping_size*(len(scale))+2] + layers[1:]
    else:
  ...
  def forward(self, x):
    ...
    if self.use_fourier_features:
      t_f,x_f,zeta_f = self.fourier_embedding(x)
    t_x_f = torch.cat([t_f, x_f, zeta_f], dim=1)
...
# find index of seta = 0
seta_idx = 1#np.where(seta==0.00)[0]
print('Seta index:', seta_idx)
ring1_params =  {
...
P_x = np.max(np.abs(ring1.theta))-np.min(np.abs(ring1.theta))
P_zeta = np.max(ring1.seta)
print(P_x, P_t, P_zeta)
...
# %%
training_params = {
    'layers': [3, 256, 256, 256, 256, 256, 2],  # Example architecture; modify as needed
...
# set wandb run name
wandb.run.name = 'LLE_PINN_MSFFN'
# %%
# # generate tau and t
T0, Theta0, u_init, v_init, seta_init, T_lb, Theta_lb, T_ub, Theta_ub, T, Theta, seta_array, F = \
  ring1.gen_tau_t_lhs(N0=200,Nb=10, Nf=50000, seta_idx=seta_idx)
print(T_lb.shape, Theta_lb.shape, T_ub.shape, Theta_ub.shape, T.shape, Theta.shape, seta_array.shape, F.shape, u_init.shape, v_init.shape)
...
def validate(network, T_val, Tau_val,seta_val, T_lb=None, Tau_lb=None, T_ub=None, Tau_ub=None, T0=None, Tau0=None):
    ...
    if ring1.seta.max() == ring1.seta.min():
        cax1 = ax1.imshow(np.abs(h), cmap='jet', origin='lower', aspect='auto', extent=[ring1.tau.min(), ring1.tau.max(), \
                                                                                        ring1.theta.min(), ring1.modes.max()])
