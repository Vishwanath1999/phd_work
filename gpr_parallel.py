...
y_train = torch.tensor([compute_mse(x[0].item(), x[1].item()) for x in x_train], dtype=torch.float32)
...
for i in tqdm(range(num_iterations),ncols=120, desc='Optimization Loop'):
    ...
    result = minimize(acquisition, x0=x_next.flatten().cpu().numpy(), bounds=bounds, method='L-BFGS-B', options={'maxiter': 100})
    x_next = torch.tensor(result.x, dtype=torch.float32)
    # print success of result
    print(f"Success: {result.success}, Message: {result.message}")

    # Evaluate the reward at the new point
    y_next = torch.tensor([compute_mse(x_next[0].item(), x_next[1].item())], dtype=torch.float32)
