...
class Ring:
    def __init__(self, ring_parameters) -> None:
        ...
        self.D3 = ring_parameters['D3'] # Third order dispersion [Hz]
        ...
        self.dint = (self.d2 / 2) * self.mu**2 + (self.d3/6) * self.mu**3 # Normalized integrated dispersion omega_mu - omega_0 - D1*mu
        self.dint /= 10
    ...
    def numerical_simulation(self, parameters, simulation_options, dseta_forward=np.array([]), amu_forward=np.array([]), theta_forward=np.array([])):
        ...
        amu = np.zeros((dseta.size, self.N), dtype=np.complex128) # 2D array to store normalized fields
        ...
        theta = np.zeros(len(dseta), dtype=np.complex128) # 1D array to store initial normalized variation of temperature
        ...
        def LLE(tau, y): 
            ...
            damu_ = -(1 + 1j * (dseta_current + self.dint) - aux * theta_) * amu_ + 1j * ifft(np.abs(fft(amu_))**2 * fft(amu_)) + self.f # LLE
        ...
        for i in tqdm(range(dseta.size - 1), ncols=120):
    ...
    def plot_results(self, dseta_forward, amu_forward, dseta_backward=np.array([]), amu_backward=np.array([]), dseta_snap=0):
        ...
        def plot_intracavity_power(dseta, amu, dseta_snap, ax, line_color='blue'):
            ...
            temp = np.zeros_like(ring_circumference, dtype=np.complex128)
