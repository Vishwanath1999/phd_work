...
class AssymPrioritizedReplayBuffer:
    def __init__(self, max_size, input_shape, n_actions, priv_dim=1, alpha=0.6):
        self.mem_size = max_size
        self.mem_cntr = 0
        self.alpha = alpha
        self.epsilon = 1e-6  # small constant to avoid zero priority
        ...
        self.new_state_memory = np.zeros((self.mem_size, *input_shape), dtype=np.float32)
        self.action_memory = np.zeros((self.mem_size,n_actions), dtype=np.float32)
        self.priv_memory = np.zeros((self.mem_size, priv_dim), dtype=np.float32)
        self.reward_memory = np.zeros(self.mem_size, dtype=np.float32)
        self.terminal_memory = np.zeros(self.mem_size, dtype=bool)

        self.priorities = np.zeros((self.mem_size,), dtype=np.float32)

    def store_transition(self, state, action, priv, reward, state_, done):
        '''
        Store a transition in the replay buffer.
    ...
    def sample_buffer(self, batch_size, beta=0.4):
        ...
        states = self.state_memory[indices]
        actions = self.action_memory[indices]
        privs = self.priv_memory[indices]
        rewards = self.reward_memory[indices]
        states_ = self.new_state_memory[indices]
        dones = self.terminal_memory[indices]

        return states, actions, privs, rewards, states_, dones, indices, weights
