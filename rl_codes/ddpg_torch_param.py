...
class Agent:
    ...
    def __init__(self, alpha, beta, input_dims, tau=0.001, gamma=0.99, n_actions_cont=1, n_actions_disc=3\
        ...
        self.critic = CriticNetwork(beta, input_dims, layer1_size, layer2_size, n_actions_cont, n_actions_disc, 'critic')
        ...
        self.target_critic = CriticNetwork(beta, input_dims, layer1_size, layer2_size, n_actions_cont, n_actions_disc, 'target_critic')
    ...
    def learn(self,step=None):
        ...
        critic_value_ = self.target_critic.forward(states_, cont_actions_, disc_actions_).view(-1)

        critic_value = self.critic.forward(states, cont_actions, disc_actions).view(-1)
        ...
        actor_loss = -self.critic.forward(states, cont_actions, disc_actions).mean()
