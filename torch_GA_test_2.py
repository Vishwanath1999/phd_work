...
# 10.11486226 -3.22604161
del_omega_init = 10.11486226*un_norm_kappa#sim_tensor['domega_init']
del_omega_end = -3.22604161*un_norm_kappa#sim_tensor['domega_end']
del_omega_stop = -3.22604161*un_norm_kappa#sim_tensor['domega_stop']
...
def initialize_del_omega_all(fine, dwell_steps, Nt, del_omega_init, del_omega_end, rescale_and_quantize, device):
    ...
    del_omega_all = torch.clip(del_omega_all, min=min_del_omega, max=max_del_omega)
...
# %%
def LLE(fine, dwell_steps, pump_power, progress_bar=False, ton='strong', save_dir=None, save_plots=False,run_id=0):
    """
    ...
    for ii in range(len(fpmp)):
        ...
        Ain[ii] = torch.fft.ifft(torch.fft.fftshift(Ein[ii],dim=0),dim=0)*torch.exp(-1j*phi_pmp[ii])
    # Run the main solver with the updated del_omega_all
    saved_data, num_step = MainSolver(Nt, saved_data, u0, del_omega_all, Ain, show_progress=progress_bar, ton=ton)
    np_dict = tensor_to_dict(saved_data)
    Acav = np.sqrt(np_dict['alpha']/2)* np_dict['u_probe']*np.exp(1j*np.pi)/np.sqrt(np_dict['u_probe'].shape[1])
    # Ecav = np.fft.fftshift(np.fft.fft(Acav, axis=1),axes=1)/np.sqrt(np_dict['u_probe'].shape[1])
...
# %%
def run_single_simulation_set(run_id, fine, dwell_steps, pump_power, base_save_dir):
    """
    Runs the simulation for all ton_cases for a single run_id and saves the results.
    """
    run_save_dir = os.path.join(base_save_dir, f'run_{run_id}')
    os.makedirs(run_save_dir, exist_ok=True)
    
    print(f"Starting Run ID: {run_id}")
    ton_cases = ['weak', 'moderate', 'strong']
    with tqdm(total=len(ton_cases), desc=f'Run {run_id}', position=run_id, leave=False) as pbar:
        for ton_case in ton_cases:
            print(f"Run ID {run_id}: Running simulation with ton='{ton_case}', fine={rescale_and_quantize(fine)/(2*np.pi*1e9)} GHz, dwell_steps={dwell_steps}, pump_power={rescale_power(pump_power)} W")
            
            # Disable progress bar for parallel runs to keep output clean
            spec, _ = LLE(fine, dwell_steps, pump_power, progress_bar=False, ton=ton_case, save_dir=run_save_dir, save_plots=True, run_id=run_id)
            
            # print(f"Run ID {run_id}, ton={ton_case}: Number of steps to achieve soliton: {num_steps}")
            
            plt.figure(figsize=(14, 4))
            plt.vlines(np.arange(-220, 221, 1), -60 * np.ones(441), spec, label='Optimized Spectrum', color='blue')
            plt.vlines(np.arange(-220, 221, 1), -60 * np.ones(441), desired_spectrum_dBm, label='Desired Spectrum', color='red', alpha=0.5)
            plt.title(f'Optimized Spectrum (ton={ton_case}, run={run_id})', fontsize=18, fontweight='bold')
            plt.xlabel('Mode no.', fontsize=16)
            plt.ylabel('Power (dBm)', fontsize=16)
            plt.xticks(fontsize=16)
            plt.yticks(fontsize=16)
            ...
            plt.savefig(os.path.join(run_save_dir, f'optimized_spectrum_{ton_case}.png'))
            plt.savefig(os.path.join(run_save_dir, f'optimized_spectrum_{ton_case}.svg'), format='svg')
            plt.close()  # Close the figure to free memory
            
            # mse = fitness(spec)
            # print(f"Run ID {run_id}, ton={ton_case}: Mean Squared Error (MSE): {mse:.4f}")
            pbar.update(1)

    print(f"Finished Run ID: {run_id}. Results saved in {run_save_dir}")
...
if __name__ == '__main__':
    ...
    parallel_runs_dir = os.path.join(save_dir, 'parallel_runs')
    os.makedirs(parallel_runs_dir, exist_ok=True)

    num_runs = 10
    pool_size = min(num_runs, os.cpu_count())

    with mp.Pool(processes=pool_size) as pool:
        args = [(i, fine, dwell_steps, pump_power, parallel_runs_dir) for i in range(num_runs)]
        pool.starmap(run_single_simulation_set, args)
