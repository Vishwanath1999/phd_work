...
ring1_params =  {
                        'N': 400, # Number of modes. It must be odd!
...
# %%
# # generate tau and t
T0, Theta0, u_init, v_init, T_lb, Theta_lb, T_ub, Theta_ub, T, Theta, seta_array, F = ring1.gen_tau_t_lhs(N0=200,Nb=400, Nf=50000)
print(T_lb.shape, Theta_lb.shape, T_ub.shape, Theta_ub.shape, T.shape, Theta.shape, seta_array.shape, F.shape, u_init.shape, v_init.shape)
...
data_tensors = to_tensor_device({'T0': T0, 'Theta0': Theta0, 'u_init': u_init, 'v_init': v_init, \
    'T_lb': T_lb, 'Theta_lb': Theta_lb, 'T_ub': T_ub, 'Theta_ub': Theta_ub, 'T_sampled': T, 'Theta_sampled': Theta,\
         'seta_array': seta_array, 'F': F}, device)
...
lamda_b, lamda_pde, lamda_init = 50.0, 1.0, 100.0  # Initial lambda values
...
for epsilon in training_params['epsilons']:
  ...
  for epoch in tqdm(range(epochs), ncols=120):
    ...
    u_init_, v_init_ = data_tensors['u_init'], data_tensors['v_init']
    T_lb_, Theta_lb_ = data_tensors['T_lb'], data_tensors['Theta_lb']
    T_ub_, Theta_ub_ = data_tensors['T_ub'], data_tensors['Theta_ub']
